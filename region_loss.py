...
def build_targets(pred_corners, target, num_keypoints, num_anchors, num_classes, nH, nW, noobject_scale, object_scale,
                  sil_thresh, seen):
    nB = target.size(0)
    nA = num_anchors
    nC = num_classes
    ...
    for b in range(nB):
        ...
        for t in range(50):
            ...
            cur_confs = torch.max(cur_confs, corner_confidences(cur_pred_corners, cur_gt_corners)).view_as(conf_mask[
...
class RegionLoss(nn.Module):
    ...
    def forward(self, output, target, epoch):
        ...
        conf = torch.sigmoid(
            output.index_select(2, Variable(torch.cuda.LongTensor([2 * num_keypoints]))).view(nB, nA, nH, nW))
        cls = output.index_select(2, Variable(
            torch.linspace(2 * num_keypoints + 1, 2 * num_keypoints + 1 + nC - 1, nC).long().cuda()))
        ...
        nGT, nCorrect, coord_mask, conf_mask, cls_mask, txs, tys, tconf, tcls = \
            build_targets(pred_corners, target.data, num_keypoints, nA, nC, nH, nW, self.noobject_scale,
        ...
        print('%d: nGT %d, recall %d, proposals %d, loss: x %f, y %f, conf %f, total %f' % (
            self.seen, nGT, nCorrect, nProposals, loss_x.item(), loss_y.item(), loss_conf.item(), loss.item()))
        # used in previous pytorch version
        # self.seen, nGT, nCorrect, nProposals, loss_x.data[0], loss_y.data[0], loss_conf.data[0], loss.data[0]))
